...
df = pd.read_csv('/Users/jbachlombardo/Documents/Tere International/Clients/PIPD/Ad data/Christians results/Christians_28Feb_raw.csv')
df[['Ad name', 'C2A']] = df['Ad name'].str.split('_', expand = True)
df['C2A'] = df['C2A'].fillna('None')

# df = df[df['Campaign name'] == 'Free Ahed Tamimi']
# df = df[(df['Ad name'] == 'Basic need') | (df['Ad name'] == 'No farms') | (df['Ad name'] == 'Own water') | (df['Ad name'] == 'Inequality') | (df['Ad name'] == 'Main')]
...
df_scoring_ad_aud = df_scoring_ad_aud.sort_values('Score', ascending = False).sort_index(level = 'Ad set name', sort_remaining = False) #SORT WITHIN AD SET
df_scoring_ad_aud_by_2 = df_scoring_ad_aud.sort_values('Score', ascending = False).sort_index(level = ['Ad set name', 'Ad name'], sort_remaining = False) #SORT BY C2A WITHIN AD SET *AND* AD
...
writer = pd.ExcelWriter('/Users/jbachlombardo/Documents/Tere International/Clients/PIPD/Ad data/Christians results/Christians_initial_scoring_28Feb.xlsx')
